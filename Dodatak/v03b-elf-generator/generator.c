...
int main()
{
    struct ElfFile
    {
        ...
        Elf64_Sym symtab[8];                     // +padd [128 - 319] :  8 * sizeof(Elf64_Sym) = 8 * 24
        uint8_t strtab[SECTION_SIZE_STRTAB];     //       [320 - 371] : 52 * sizeof(uint8_t) = 52 * 1
        Elf64_Rela relaText[1];                  // +padd [376 - 399] :  1 * sizeof(Elf64_Rela) = 1 * 24
        uint8_t shstrtab[SECTION_SIZE_SHSTRTAB]; //       [400 - 448] : 49 * sizeof(uint8_t) = 49 * 1
        Elf64_Shdr sectionHeaderTable[8];        //       [456 - 967] :  8 * sizeof(Elf64_Shdr) = 8 * 64
    } elfFile = {
        ...
        .elfHeader = {
            ...
            .e_shoff = (void *)&elfFile.sectionHeaderTable - (void *)&elfFile, /* Section header table file offset */
